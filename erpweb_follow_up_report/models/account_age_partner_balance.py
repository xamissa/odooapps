...
def _build_periods(date_to, step_days):
    def md(days):
        return fields.Date.to_string(date_to - relativedelta(days=days))

    periods = [(False, fields.Date.to_string(date_to))]  # period0: current
    for i in range(1, 8):
        start = (i - 1) * step_days + 1
        end = i * step_days
        periods.append((md(start - 1), md(end)))
    periods.append((md(7 * step_days), False))  # period8: older
    return periods
...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        period_labels = {
            'period0': _('At Date'),
            **{f'period{i}': _('%s Days') % (step * i) for i in range(1, 8)},
            'period8': _('Older'),
        }
        for col in options.get('columns', []):
            expr_label = col.get('expression_label')
            if expr_label in period_labels:
                col['name'] = period_labels[expr_label]
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        step = self._get_aging_step()
        periods = _build_periods(date_to, step)
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT apt.name -> 'en_US') AS invoice_payment_term_id,
            ...
            LEFT JOIN account_payment_term apt ON apt.id = move.invoice_payment_term_id
